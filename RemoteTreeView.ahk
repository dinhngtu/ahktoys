...
class ProcessHandle {
    ...
    __New(pid, priv := this.PROCESS_VM_OPERATION | this.PROCESS_VM_READ | this.PROCESS_VM_WRITE | this.PROCESS_QUERY_INFORMATION) {
        this.hProcess := OpenProcess(priv, false, pid)
        if !this.hProcess
            throw OSError(, "OpenProcess")
        ; Try to determine the bitness of the remote tree-view's process
        this.is32Bit := A_PtrSize = 8 ? False : True
        if (A_Is64bitOS) && DllCall("Kernel32.dll\IsWow64Process", "Ptr", this.hProcess, "int*", &WOW64 := true)
            this.is32Bit := WOW64
    }
    dispose() {
        if this.hProcess
...
class RemoteBuffer {
    ...
    __New(hProcess, size, type := this.MEM_COMMIT, prot := this.PAGE_READWRITE, addr := 0) {
        this.hProcess := hProcess
        this.size := size
        this.mem := VirtualAllocEx(this.hProcess, addr, size, type, prot)
        if !this.mem
            throw OSError(, "VirtualAllocEx")
    }

    dispose() {
        if this.mem
            VirtualFreeEx(this.hProcess, this.mem, 0, this.MEM_RELEASE)
        this.mem := 0
    }

    put(buf) {
        return WriteProcessMemory(this.hProcess, this.mem, buf, Min(buf.Size, this.size))
    }

    get(buf) {
        return ReadProcessMemory(this.hProcess, this.mem, buf, Min(buf.Size, this.size))
    }
...
class RemoteTreeView {
    ...
    GetText(pItem) {
        ...
        If (proc.is32Bit) {
            ...
            NumPut("UInt", _txt.mem, tvi, 16)
            NumPut("UInt", 260, tvi, 20)
        }
        Else {
            NumPut("UInt64", pItem, tvi, 8)
            NumPut("UInt64", _txt.mem, tvi, 24)
            NumPut("UInt", 260, tvi, 32)
        }

        _tvi.put(tvi)
        SendMessage(this.TVM_GETITEMW, 0, _tvi.mem, , "ahk_id " this.TVHwnd)
        txt := Buffer(260 * 2 + 2, 0)
        _txt.get(txt)

        return StrGet(txt)
    }

    GetItemRect(pItem, code := true) {
        ProcessId := WinGetPID("ahk_id " this.TVHwnd)
        proc := ProcessHandle(ProcessId)
